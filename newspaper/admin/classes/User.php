...
class User extends Database
{
    ...
    {
        $sql = "SELECT COUNT(*) as email_count FROM school_publication_users WHERE email = ?";
    ...
    public function registerUser($username, $email, $password, $is_admin = true)
    {
        ...
        try {
            $result = $this->executeNonQuery($sql, [$username, $email, $hashed_password, (int)$is_admin]);
            return $result > 0;
    ...
    {
        $sql = "SELECT user_id, username, password, is_admin FROM school_publication_users WHERE email = ? AND is_admin = 1";
